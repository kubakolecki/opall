...
- There are two more sample datasets: 3 and 4. Dataset 3 is analogous to dataset 1. Dataset 4 is analogous to dataset 2. The difference is in how the uncertainty of point measurements is provided. For datasets 3 and 4, the uncertainties are provided as 3 x 3 covariance matrices. In other words, we handle the anisotropic noise of point measurements. For example, if points are measured by a laser scanner with high accuracy distance measurements but with large laser beam divergence, the measured point will have higher position uncertainty in the direction perpendicular to the ray compared to uncertainty along the ray. Thus, typically, we will see some nonzero off-diagonal entries in the given covariances. Opall will handle covariance matrices correctly in the process of residual computation.
...
dataset 3:
```bash
./opall_build/bin/opall_solver_app/opall_solver_app -p ./opall/sample_data/simulation_03/solver_input/poses.txt -m ./opall/sample_data/simulation_03/solver_input/lidar_measurements.txt -c ./opall/sample_data/simulation_03/solver_config.cfg -o ./opall/sample_data/simulation_03/solver_output -r
```
dataset 4:
```bash
